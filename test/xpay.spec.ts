...
const mockedAxios = AxiosWrapper as jest.Mocked<typeof AxiosWrapper>;

describe("preparePayment method execute successfuly", () => {
  beforeAll(() => {
    mockedAxios.prepareAmount.mockResolvedValue({
      total_amount: 50,
...
describe("makePayment method execute successfuly", () => {
  beforeAll(() => {
    ...
    mockedAxios.pay.mockResolvedValue({
      iframe_url: null,
      transaction_id: 4647,
  ...
  test("sends a customfield object with the payload if passed to the method", async () => {
    ...
    return payment
      ...
      .then((data) => {
        expect(mockedAxios.pay).toHaveBeenCalledWith(
          {
...
describe("getTransaction method execute successfuly", () => {
  beforeAll(() => {
    mockedAxios.getTransactionInfo.mockResolvedValue({
...
describe.only("makePayment method throws", () => {
  beforeAll(() => {
    mockedAxios.prepareAmount.mockResolvedValue({
      KIOSK: { total_amount: 50, total_amount_currency: "EGP" },
    });
